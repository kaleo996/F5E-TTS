...
class AugmentWAV(object):
    ...
    def __init__(self, musan_path, rir_path):
        ...
        self.noiselist  = {}
        if os.path.isdir(musan_path) and os.path.isdir(rir_path):
            self.use_h5 = False
        ...
        elif os.path.isfile(musan_path) and os.path.isfile(rir_path):
            self.use_h5 = True
            self.noi_h5 = musan_path
            self.rir_h5 = rir_path
            with h5py.File(musan_path, "r") as f:
                self.noi_utt_ids = f['utt_ids'][:].astype(numpy.str)
                self.noi_positions = f['positions'][:]
                #self.noi_wav = f["wav"][:]
            self.noi_wav = h5py.File(musan_path, 'r')["wav"]
            for i in range(len(self.noi_utt_ids)):
                utt_id = self.noi_utt_ids[i]
                if utt_id == 'speec': utt_id = 'speech'
                assert utt_id in self.noisetypes
                if not utt_id in self.noiselist:
                    self.noiselist[utt_id] = []
                self.noiselist[utt_id].append(i)
            with h5py.File(rir_path, "r") as f:
                self.rir_utt_ids = f['utt_ids'][:].astype(numpy.str)
                self.rir_positions = f['positions'][:]
                #self.rir_wav = f["wav"][:]
            self.rir_wav = h5py.File(rir_path, 'r')["wav"]
        else:
            raise ValueError
    ...
    def additive_noise(self, noisecat, audio):
        ...
        if not self.use_h5:
            for noise in noiselist:
                noiseaudio, sample_rate = torchaudio.load(noise)
...
#            noi_wav = h5py.File(self.noi_h5, 'r')["wav"]
            for noise in noiselist:
                pos = self.noi_positions[noise]
                pos_s, pos_e = pos[0], pos[0]+pos[1]
       #         import ipdb; ipdb.set_trace()
                noiseaudio = self.noi_wav[pos_s: pos_e].astype(numpy.float32)[None,:]
                noiseaudio /= (1 << 15)
                noise_length = noiseaudio.shape[1]
                if noise_length <= audio_length:
                    shortage    = audio_length - noise_length + 1
    ...
    def reverberate(self, audio):
        ...
        if not self.use_h5:
            ...
            rir_file = random.randint(0, len(self.rir_utt_ids)-1)
            pos = self.rir_positions[rir_file]
            pos_s, pos_e = pos[0], pos[0]+pos[1]
#            with h5py.File(self.rir_h5, 'r') as f:
            rir = self.rir_wav[pos_s:pos_e].astype(numpy.float32)[None,:]#rir audio
            rir /= (1 << 15)
