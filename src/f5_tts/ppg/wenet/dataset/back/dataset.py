...
def Dataset(data_type,
    ...
    if data_type == 'shard':
        ...
        #{key, wav, txt, sample_rate}, key is utt name
        dataset = Processor(dataset, processor.tar_file_and_group)
    else:
        dataset = Processor(dataset, processor.parse_raw)

    # convert token into ideces
    # Iterable[{key, wav, txt, tokens(list), label(list), sample_rate}]
    ...
    # Iterable[{key, feat(flen,mel), label}]
...
if __name__ == '__main__':
    ...
    use_seg="false"
    use_emb="false"
    train_config=f"conf/train_conformer_subSample_false_emb_false.yaml"
...
#    for item in cv_dataset: # train_dataset: # 
    for item in train_dataset: # 
        if len(item) == 7:
            ...
            sorted_keys, padded_feats, padding_labels, feats_lengths, label_lengths = item
        print(sorted_keys)
        print(feats_lengths)
        print(label_lengths)
        # import ipdb; ipdb.set_trace()
